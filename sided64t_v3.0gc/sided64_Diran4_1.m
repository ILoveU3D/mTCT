...
load("gc.mat");
bias = 100/dSliceInterval;
sou_pos = rotateAxis(sou_pos);
sou_pos(:,:,3) = sou_pos(:,:,3) - bias;
det_pos = rotateAxis(det_pos);
det_pos(:,:,3) = det_pos(:,:,3) - bias;
det_u = rotateAxis(det_u);
det_v = rotateAxis(det_v);
scatter3(sou_pos(:,1,1),sou_pos(:,1,2),sou_pos(:,1,3));
hold on;
scatter3(det_pos(:,1,1),det_pos(:,1,2),det_pos(:,1,3));
hold off;
